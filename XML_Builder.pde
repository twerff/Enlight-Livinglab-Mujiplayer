...
String name = "XML used for MULI test (Floorplan interface)";
...
void createXML(String filename)
{
  ...
  {
    if (getLuminaire(i).getName().contains("TX")) t = new LuminaireXML(getLuminaire(i));
    else
    {
      l = new LuminaireXML(getLuminaire(i));
      
      //SELF
      r = new RuleXML("presence detected from this luminaire");
      r.addTriggerEvent("PresenceDetected", getLuminaire(i).getAddress(), "Self");
      //r.addAction("LevelActivation", getLuminaire(i).getLevel("lvl_work").getTimeOut(), true, getLuminaire(i).getLevel("lvl_work").getLevel());
      r.addAction("UserEvent", 99);
      
      configureParameter("PresenceSensorDevice_PresenceReportingInterval",600);
      configureParameter("PresenceSensorDevice_AbsenceTime",ABSENCETIME);
      configureParameter("PresenceSensorDevice_PresenceLevel",1);
    }
  }
...
public class LuminaireXML extends XML
{
  ...
  private void setDefs()
  {
    ...
    for (int i=1; i < l.getNrOfLevels ()+1; i++)
    {
  ...
  public void init()
  {
    ...
    {
      r.addAction("LevelActivation", l.getLevel("lvl_work").getTimeOut(), true, l.getLevel("lvl_work").getLevel());
...
void pcRules(Luminaire l)
{
  ...
  r.addAction("SetColorTemperature", 1, 2);
  ...
  r.addAction("SetDimmingLevel", 1, 2);
  ...
  r.addAction("LevelActivation", true, 1, 2);
  //off
  r = new RuleXML("Off Received from PC");
  r.addTriggerEvent("OnOffChanged", address, "PC");
  r.addCondition(1, "EQ", 0);
  r.addAction("LevelActivation", false, 1, 2);

  //r = new RuleXML("CT on scope changed from PC");
  //r.addTriggerEvent("event15", address, "PC");
  //r.addCondition();
  //r.addAction("SetColorTemperature",1,2);

  //  int lvl = 1;
  //  r = new RuleXML("DimLevelChanged for level " + lvl + "from PC");
  //  r.addTriggerEvent("DimLevelChanged", ADDRESS_LVL+lvl, "LVL"+lvl);
  //  r.addAction("SetDimmingLevel",lvl);
  //  
  //  lvl=2;
  //  r = new RuleXML("DimLevelChanged for level " + lvl + "from PC");
  //  r.addTriggerEvent("DimLevelChanged", ADDRESS_LVL+lvl, "LVL"+lvl);
...
public class RuleXML extends XML
{
  ...
  public void addAction(String actionType, int...levels)
  {
    ...
    else if (actionType.equals("UserEvent"))
    {
      ...
      //actiontype.setString("Address", ADDRESS_DONGLE);
