...
String HUE_KEY = "w5C1y3vjlnhOXW4YGIMokiRNxQoNtyzTQ9Z5fENN";
String HUE_IP  = "192.168.1.101";
int NUM_HUE_LAMPS = 3;

ArrayList<HueLamp> _hueLamps = new ArrayList<HueLamp>();
HueHub hub;

void setupHue(String ip, String kkey, int numHue)
{
  NUM_HUE_LAMPS = numHue;
  HUE_KEY = kkey;
  setupHue(ip);
}
...
void setupHue(String ip)
{
  ...
  for (int i = 0; i < NUM_HUE_LAMPS; i++)
  {
    ...
    //add the positions
    _hueLamps.get(i).setPosition(1, i+1);
  }
...
class HueHub 
{
  private final String KEY = HUE_KEY; // "secret" key/hash
  private String IP = HUE_IP; // ip address of the hub
  private final boolean ONLINE = true; // for debugging purposes, set to true to allow communication
